...
/**
 * This class defines the structure of the 'googleshopping_taxonomy' table.
 *
 *
 *
 ...
 *
 */
class GoogleshoppingTaxonomyTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'GoogleShopping.Model.Map.GoogleshoppingTaxonomyTableMap';
     ...
     */
    const TABLE_NAME = 'googleshopping_taxonomy';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\GoogleShopping\\Model\\GoogleshoppingTaxonomy';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'GoogleShopping.Model.GoogleshoppingTaxonomy';
     ...
     */
    const ID = 'googleshopping_taxonomy.ID';

    /**
     * the column name for the THELIA_CATEGORY_ID field
     */
    const THELIA_CATEGORY_ID = 'googleshopping_taxonomy.THELIA_CATEGORY_ID';
    ...
    {
        // attributes
        $this->setName('googleshopping_taxonomy');
        $this->setPhpName('GoogleshoppingTaxonomy');
        $this->setClassName('\\GoogleShopping\\Model\\GoogleshoppingTaxonomy');
    ...
    {
        return $withPrefix ? GoogleshoppingTaxonomyTableMap::CLASS_DEFAULT : GoogleshoppingTaxonomyTableMap::OM_CLASS;
    }
     ...
     * @return array (GoogleshoppingTaxonomy object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = GoogleshoppingTaxonomyTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = GoogleshoppingTaxonomyTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + GoogleshoppingTaxonomyTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = GoogleshoppingTaxonomyTableMap::OM_CLASS;
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            GoogleshoppingTaxonomyTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = GoogleshoppingTaxonomyTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = GoogleshoppingTaxonomyTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(GoogleshoppingTaxonomyTableMap::DATABASE_NAME)->getTable(GoogleshoppingTaxonomyTableMap::TABLE_NAME);
    }
    ...
    {
      $dbMap = Propel::getServiceContainer()->getDatabaseMap(GoogleshoppingTaxonomyTableMap::DATABASE_NAME);
      if (!$dbMap->hasTable(GoogleshoppingTaxonomyTableMap::TABLE_NAME)) {
        $dbMap->addTableObject(new GoogleshoppingTaxonomyTableMap());
      }
    }

    /**
     * Performs a DELETE on the database, given a GoogleshoppingTaxonomy or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or GoogleshoppingTaxonomy object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \GoogleShopping\Model\GoogleshoppingTaxonomy) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(GoogleshoppingTaxonomyTableMap::DATABASE_NAME);
            $criteria->add(GoogleshoppingTaxonomyTableMap::ID, (array) $values, Criteria::IN);
        }

        $query = GoogleshoppingTaxonomyQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) { GoogleshoppingTaxonomyTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) { GoogleshoppingTaxonomyTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return GoogleshoppingTaxonomyQuery::create()->doDeleteAll($con);
    }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from GoogleshoppingTaxonomy object
        }
...
} // GoogleshoppingTaxonomyTableMap
// This is the static code needed to register the TableMap for this table with the main Propel class.
//
GoogleshoppingTaxonomyTableMap::buildTableMap();
